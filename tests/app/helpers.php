    ...
    if(!empty($superCategoryList)){
        foreach($superCategoryList as $key => $superCategory){
            ...
            if (!empty($superSubCategoryList->toArray()))
            {
                ...
                {
                    $categoryList = Category::where('deleted_at','')->where('supersub_cat_id',$superSubCategory->id)->get();
                    if (!empty($categoryList->toArray())) 
                    {                            
                        foreach ($categoryList as $value) 
                        {                                
                            $subCategoryList = SubCategory::where(['deleted_at' =>'','category_id' => $value->id])->get();                                
...
{
    $notificationList = Notification::where('deleted_at','')->where('receiver_type','admin')->where('is_read','0')->orderBy('id','DESC')->limit(5)->get();
