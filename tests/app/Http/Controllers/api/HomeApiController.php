...
class HomeApiController extends ApiBaseController
{
    ...
    public function filterList(Request $request)
    {
        try
        {
            ...
            $selectProduct = Product::where('deleted_at','')->where('product_status','1')->whereRaw('used_quantity < quantity')->whereIn('id',$productId)->get();
            ...
            $minPrice = Product::selectRaw('MIN(CAST(price as DECIMAL(10))) as minPrice')->where('deleted_at', '')->where('product_status', '1')->whereRaw('used_quantity < quantity');
            if ($bannerId)
            {
                $minPrice->whereIn('id',$productId);
            }
            $minPrice = $minPrice->first();
            $maxPrice = Product::selectRaw('MAX(CAST(price as DECIMAL(10))) as maxPrice')->where('deleted_at', '')->where('product_status', '1')->whereRaw('used_quantity < quantity');
            if ($bannerId)
            {
