...
class AdminController extends Controller
{
    ...
    { 
        ...
        $ongoingOfferCount = Offer::where('deleted_at', '')->where('offer_status','1')->whereDate('start_date','<=',Carbon::today())->whereDate('end_date','>=',Carbon::today())->count();
        ...
        $todayTicketRaiseCount = Order::where('cancel_id','!=','0')->where('return_date',$date)->count();
        ...
        $todayReturnOrderCount = Order::where('deleted_at','')->where('order_status','5')->where('return_date', $date)->count();
        $todayCashCollected = Order::where('deleted_at','')->where('payment_type','1')->where('order_status','4')->whereDate('delivered_date', Carbon::today())->sum('total_amount');

        $todayAvailableProductCount = Product::where('deleted_at', '')->whereDate('created_at', Carbon::today())->where('product_status','1')->count();
        
        return view('admin.dashboard',compact('totalCustomer','totalDeliveryAssociate','ticketRaiseCount','ongoingOfferCount','availableProductCount','canceledOrderCount','todayCustomer','todayDeliveryAssociate','todayTicketRaiseCount','todayOngoingOfferCount','todayCanceledOrderCount','todayAssignOrder', 'deliveredOrderCount', 'returnOrderCount', 'todayDeliveredOrderCount', 'todayReturnOrderCount','todayCashCollected','todayAvailableProductCount'));
    }
